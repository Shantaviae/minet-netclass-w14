...
ostream &ARPPacket::Print(ostream &os) const
{
  ...
  os << "ARP(hwaddrspace="<<hwadxspace<<", protoaddrspace="<<protoadxspace;
  os << ", hwlen="<<(int)hwlen<<", protolen="<<(int)protolen;
  os << ", opcode="<< (opcode==Request ? "Request" : "Reply");
  os << ", senderhw="<<senderhw<<", senderip="<<senderip;
  os << ", targethw="<<targethw<<", targetip="<<targetip<<")";
  
  return os;
}


ARPRequestResponse::ARPRequestResponse() : ipaddr(0U), ethernetaddr("00:00:00:00:00:00"), flag(RESPONSE_UNKNOWN)
{}

ARPRequestResponse::ARPRequestResponse(const ARPRequestResponse &rhs) : ipaddr(rhs.ipaddr), ethernetaddr(rhs.ethernetaddr), flag(rhs.flag)
{}

ARPRequestResponse::ARPRequestResponse(const IPAddress &ip, 
				       const EthernetAddr &hw,
				       const Flag        f): ipaddr(ip), ethernetaddr(hw), flag(f)
{}
...
ostream & ARPRequestResponse::Print(ostream &os) const
{
  os << "ARPRequestResponse(ipaddr="<<ipaddr<<", ethernetaddr="<<ethernetaddr<<", flag="<<((int)flag)<<" "<< 
...
ostream & operator<< (ostream &os, pair<IPAddress,ARPRequestResponse> &pair)
{
  os << (pair.second) << " " ;
  return os;
}

ostream & ARPCache::Print(ostream &os) const
{
  os << "ARPCache(size="<<data.size()<<" contents={";
  for_each(data.begin(),data.end(),PrintFunc<pair<IPAddress,ARPRequestResponse> >(os));
  os << "})";
  return os;
}

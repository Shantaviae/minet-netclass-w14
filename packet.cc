...
void Packet::Serialize(const int fd) const
{
  ...
  for (deque<Header>::const_iterator p=headers.begin();p!=headers.end();p++) {
    (*p).Serialize(fd);
  }
  ...
  for (deque<Trailer>::const_iterator p=trailers.begin();p!=trailers.end();p++) {
    (*p).Serialize(fd);
  }
}
...
struct find_pred : public unary_function<T,bool> {
  TAGTYPE tag;
...
{
  deque<Header>::const_iterator i = find_if(headers.begin(), headers.end(), find_pred<Header,Headers::HeaderType>(ht));
  return *(new Header(*i));
}
...
{
  deque<Trailer>::const_iterator i = find_if(trailers.begin(), trailers.end(), find_pred<Trailer,Trailers::TrailerType>(ht));
...
ostream & Packet::Print(ostream &os) const
{
  os << "Packet(headers={";

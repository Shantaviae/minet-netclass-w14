...
void TCPHeader::GetHeaderLen(unsigned char &l) const
{
  GetData((char*)&l,1,12);
  l=(l>>4)&0xf;
}

void TCPHeader::SetHeaderLen(const unsigned char &l, const Packet &p)
{
  unsigned char lt;
  GetData((char*)&lt,1,12);
  lt=(lt&0xf)|((l<<4)&0xf0);
  SetData((char*)&lt,1,12);
  RecomputeChecksum(p);
}
  
void TCPHeader::GetFlags(unsigned char &f) const
{
  GetData((char*)&f,1,13);
  f&=(1+2+4+8+16+32);
}

void TCPHeader::SetFlags(const unsigned char &f, const Packet &p)
{
  unsigned char ft;
...
ostream & TCPHeader::Print(ostream &os) const
{
